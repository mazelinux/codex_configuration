...
class Analyzer:
    ...
    def workflow_card(self, session: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [
            {"row": 0, "name": "Token Usage", "observation": {"total_tokens": token["total"], "input_tokens": token["input"], "output_tokens": token["output"], "cached_input_tokens": token["cached_input"], "cache_write_input_tokens": token["cache_write_input"], "reasoning_tokens": token["reasoning"], "token_samples": sum(e["kind"] == "token_count" for e in ev), "context_peak": token["context_peak"], "compactions": len(compactions)}},
